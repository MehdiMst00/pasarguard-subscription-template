...
const VALID_PROTOCOLS = ["vless://", "vmess://", "trojan://", "ss://"];

const configFetcher = async (url: string): Promise<string[]> => {
  ...
  try {
    ...
    throw new Error("Invalid base64 config response");
  }
  ...
  return decoded
    ...
    .map((l) => l.trim())
    .filter(
      (l) => l.length > 0 && VALID_PROTOCOLS.some((p) => l.startsWith(p)),
    );
};
...
export const useUserInfo = () => {
  ...
  const {
    ...
    isValidating,
    mutate,
  } = useSWR<{ data: UserInfo; headers: InfoHeaders }>(
...
export const useConfigData = () => {
  ...
  // Prefer Jinja-rendered links when available
  const initialLinks = initial?.links;

  const data: ConfigData | undefined =
    initialLinks && initialLinks.length > 0
      ? {
          links: initialLinks.filter((link) =>
            VALID_PROTOCOLS.some((p) => link.startsWith(p)),
          ),
        }
  ...
  // Same loading logic as useUserInfo
  const shouldShowLoading = isLoading && !initialLinks;
...
export const useApps = () => {
  const initialAppsArray =
    typeof window !== "undefined" ? window.__INITIAL_DATA__?.apps : undefined;
